...
namespace FrontEnd
{
	...
	public partial class DatabaseManagementWindow : UserControl
	{
		...
		private void DisplayingHomeScreen()
		{
			...
			InfoLabel.Text = "Для изменения данных в БД (базе данных) необходимо выполнить следующие действия:\n1) Выгрузить необходимую таблицу;\n2) Произвести изменения;\n3) Загрузить изменённую таблицу;\nВ таблице обязательно должна присутствовать строка с названием столбцов.\nТаблица должна содержать от одного и более студентов/ преподавателей.\nЕсли в загружаемой таблице в какой-либо из строк отсутствует ID, то ID создастся автоматически и соответствующий студент/ преподаватель будет добавлен, как новый.\nЕсли же ID указан, то произойдёт изменение данных студента/ преподавателя с соответствующим ID.";
		}
